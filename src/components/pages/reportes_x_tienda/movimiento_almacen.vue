                                ...
                                <el-col :xs="17" :sm="18" :md="20" :lg="19" :xl="20">
                                    <div>
                                        <el-select v-model="IdAlmacen" filterable placeholder="Select"
                                            ...
                                            <el-option v-for="oTienda in loAlmacen" 
                                                :key="oTienda.codigo" 
...
export default {
    ...
    data() {
        return {
            tittleModulo:'Movimiento Almacén',
            tittleReporte:'',
            //-----------------------------------
            tipoFiltro:'',
            fechaDia:'',
    ...
    methods: {
        ...
        CargarDatosIniciales(){
          ...
          ReporteClienteTiendaService.CargarAlmacen(this.usuario.usuarioLogin, this.usuario.idCliente)
            .then(res => {
                loading.close();
                if (res.status==200){
                    this.loAlmacen = res.result.loAlmacen;
                    if(this.loAlmacen.filter(obe=>obe.codigo!=0).length==1){
                        this.IdAlmacen = this.loAlmacen.filter(obe=>obe.codigo!=0)[0].codigo;
                    }
        ...
        ValidarGuardar(){
            if(this.IdAlmacen==0){
                return 'Debe seleccionar Alamcén.';
            }
        ...
        BuscarBD(){
            ...
            let fechafin = "";
            if (this.isDia){
                this.tittleReporte="REPORTE DE MOVIMIENTO ALMACÉN - POR DIA";
                ...
                this.tittleReporte="REPORTE DE MOVIMIENTO ALMACÉN - POR MES";
                ...
                this.tittleReporte="REPORTE DE MOVIMIENTO ALMACÉN - POR AÑO";
                ...
                this.tittleReporte="REPORTE DE MOVIMIENTO ALMACÉN - POR RANGO";
            ...
            let desAlmacen = this.loAlmacen.filter(obe=>obe.codigo==this.IdAlmacen)[0].descripcion;
            ReporteClienteTiendaService.MovimientoAlmacen(this.usuario.usuarioLogin, this.usuario.idCliente, this.isDia,
                                        this.isMes, this.isAnio, this.isRango, fechainicio, fechafin, 
                                        this.IdAlmacen, desAlmacen)
                .then(res => {
                    loading.close();
                    if (res.status==200){
