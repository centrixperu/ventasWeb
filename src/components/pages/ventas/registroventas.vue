    ...
    <div class="q-pa-md paddingBottom0">
      <el-container class="padding0">  
                                        ...
                                        <el-table-column type="expand" fixed="left">
                                            <template slot-scope="scope">
                                                <el-button
                                                    ...
                                                    @click="VerDetalle(scope.row.Id, scope.row)">Ver Detalle</el-button>
                                                <el-button
                                                    size="mini" type="success" icon="el-icon-printer"
                                                    @click="ImprimirComprobante(scope.row.Id, scope.row)">Imprimir Comprobante</el-button>
                                                <el-button v-show="scope.row.isNC"
                                                    size="mini" type="warning" icon="el-icon-printer"
                                                    @click="ImprimirNC(scope.row.Id, scope.row)">Imprimir NC</el-button>
                                                <el-button v-show="scope.row.isND"
                                                    size="mini" type="warning" icon="el-icon-printer"
                                                    @click="ImprimirND(scope.row.Id, scope.row)">Imprimir ND</el-button>
                                                <el-button v-show="scope.row.isAnular"
                                                    size="mini" type="danger" icon="el-icon-close"
                                                    @click="Anular(scope.row.Id, scope.row)">Anular</el-button>
                                            </template>
                                        </el-table-column>
                                    ...
                                    <el-col :xs="24" :sm="24">
                                        <div><!--class="cus-label-form-left"-->
                                            <el-button-group>
                                                <el-button
                                                    size="mini" type="primary" icon="el-icon-view"
                                                    @click="VerDetalle(oListado.Id, oListado)">Ver Detalle</el-button>
                                                <el-button
                                                    size="mini" type="success" icon="el-icon-printer"
                                                    @click="ImprimirComprobante(oListado.Id, oListado)">Imprimir Comprobante</el-button>
                                                <el-button v-show="oListado.isNC"
                                                    size="mini" type="warning" icon="el-icon-printer"
                                                    @click="ImprimirNC(oListado.Id, oListado)">Imprimir NC</el-button>
                                                <el-button v-show="oListado.isND"
                                                    size="mini" type="warning" icon="el-icon-printer"
                                                    @click="ImprimirND(oListado.Id, oListado)">Imprimir ND</el-button>
                                                <el-button v-show="oListado.isAnular"
                                                    size="mini" type="danger" icon="el-icon-close"
                                                    @click="Anular(oListado.Id, oListado)">Anular</el-button>
                                            </el-button-group>
                                        </div>
                                    </el-col>
                                </el-row>
        ...
        <el-dialog
            ...
            :visible.sync="isAnular"
            width="30%" center>
            <span>¿Esta seguro que desea anular la venta?</span>
            <span slot="footer" class="dialog-footer" v-if="isAnular">
                <el-button size="mini" type="info" @click="AnularVenta()">Anular Comprobante</el-button>
                <el-button v-show="ValidarAnularNotaCredito()" size="mini" type="warning" @click="AnularVentaNC()">Nota de Crédito</el-button>
                <el-button size="mini" type="danger" @click="isAnular = false">Cancelar</el-button>
            </span>
        </el-dialog>
        <el-dialog title="Detalle de Venta" :visible.sync="isDetalle" center fullscreen>
            <el-row :gutter="5" type="flex" class="paddingTop10 margintop10 cus-border hidden-xs-only hidden-sm-only" v-show="lobeListadoDetalle.length>0">
                <div class="cus-container-border margin0 width100">
                    <el-main class="padding5">
                        <el-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24">
                            <div>
                                <el-table :data="lobeListadoDetalleFiltrado">
                                    <el-table-column type="expand">
                                        <template slot-scope="scope">
                                            <el-row :gutter="5" class="paddingTop0">
                                                <div class="textAlignC">
                                                    <span class="fontsize15 fontkursiva">SubTotal: <b>{{ scope.row.SubTotal }}</b></span>
                                                </div>
                                            </el-row>
                                            <el-row :gutter="5" class="paddingTop0">
                                                <div class="textAlignC">
                                                    <span class="fontsize15 fontkursiva">IGV: <b>{{ scope.row.IGV }}</b></span>
                                                </div>
                                            </el-row>
                                            <el-row :gutter="5" class="paddingTop10">
                                                <div class="textAlignC">
                                                    <span class="fontsize15 fontkursiva">Nota de Venta: <b>{{ scope.row.NotaVenta }}</b></span>
                                                </div>
                                            </el-row>
                                            <el-row :gutter="5" class="paddingTop10" v-show="scope.row.Documento!=''">
                                                <div class="textAlignC">
                                                    <span class="fontsize15 fontkursiva">{{ scope.row.Documento }}: <b>{{ scope.row.NroDocumento }}</b></span>
                                                </div>
                                            </el-row>
                                        </template>
                                    </el-table-column>
                                    <el-table-column property="IdProducto" label="Id" width="60"></el-table-column>
                                    <el-table-column property="DesProducto" label="Producto"></el-table-column>
                                    <el-table-column property="Cantidad" label="Cantidad" width="80"></el-table-column>
                                    <el-table-column property="PUnitario" label="Precio Unitario" width="70"></el-table-column>
                                    <el-table-column property="Total" label="P/V sin dscto." width="75"></el-table-column>
                                    <el-table-column property="SubTotal" label="Precio Venta" width="70"></el-table-column>
                                </el-table>
                                <el-pagination
                                    @current-change="PaginarDetalle"
                                    :current-page.sync="currentPageDet"
                                    :page-size="pageSizeDet"
                                    layout="total, prev, pager, next"
                                    :total="totalrowsDet">
                                </el-pagination>
                            </div>
            ...
            <el-row :gutter="5" type="flex" class="paddingTop10 margintop10 hidden-md-only hidden-lg-only hidden-xl-only">
                <el-main class="padding5">
                    <el-row :gutter="5" type="flex" class="paddingTop5">
                        <el-col :xs="24" :sm="24">
                            <el-input type="text" ref="nomBuscarDetalle"
                                size="small" :autosize="{ minRows: 2, maxRows: 3}" 
                                placeholder="Buscar Producto" 
                                @keypress.native="keyUpBuscarDetalle"
                                v-model="nomBuscarDetalle">
                                <el-button slot="append" icon="el-icon-search"
                                        @click="BuscarDocumentoDetalle()"></el-button>
                            </el-input>
                        </el-col>
                    </el-row>
                    <el-row :gutter="5" type="flex" class="paddingTop5"
                            v-for="oListado in lobeListadoDetalleFiltrado" 
                            :key="oListado.Id" :value="oListado.Id">
                        <el-main class="padding5 cus-border">
                            <el-row :gutter="5" type="flex" class="paddingTop0">
                                <el-col :xs="24" :sm="24">
                                    <div class="cus-label-form-left width90">
                                        <span class="cus-label textAdjust textAlignL width105P">Producto :</span>
                                        <el-input v-model="oListado.DesProducto" size="mini" readonly></el-input>
                                    </div>
                                </el-col>
                            </el-row>
                            <el-row :gutter="5" type="flex" class="paddingTop5">
                                <el-col :xs="24" :sm="24">
                                    <div class="cus-label-form-left width90">
                                        <span class="cus-label textAdjust textAlignL width105P">Nota de Venta:</span>
                                        <el-input v-model="oListado.NotaVenta" size="mini" readonly></el-input>
                                    </div>
                                </el-col>
                            </el-row>
                            <el-row :gutter="5" type="flex" class="paddingTop5">
                                <el-col :xs="24" :sm="24">
                                    <div class="cus-label-form-left width90">
                                        <span class="cus-label textAdjust textAlignL width105P">{{ oListado.Documento }}:</span>
                                        <el-input v-model="oListado.NroDocumento" size="mini" readonly></el-input>
                                    </div>
                                </el-col>
                            </el-row>
                            <el-row :gutter="5" type="flex" class="paddingTop5">
                            ...
                            <el-row :gutter="5" type="flex" class="paddingTop5">
                                <el-col :xs="24" :sm="24">
                                    <div class="cus-label-form-left width90">
                                        <span class="cus-label textAdjust textAlignL width105P">Total sin IGV:</span>
                                        <el-input class="paddingRight10" v-model="oListado.SubTotal" size="mini" readonly></el-input>
                                        <span class="cus-label textAdjust textAlignL width60P">Total:</span>
                                        <el-input v-model="oListado.Total" size="mini" readonly></el-input>
                                    </div>
                                </el-col>
                            </el-row>
                        </el-main>
...
export default {
    ...
    data() {
        return {
            ...
            row:{},
            //-----------------------------------
            tittleModulo:'Registro de Ventas',
            tittleReporte:'',
            //-----------------------------------
            tipoFiltro:'',
            fechaDia:'',
    ...
    methods: {
        VerDetalle(id, row){
            VentaService.VerDetalleVenta(this.usuario.usuarioLogin, this.usuario.idCliente, id)
                .then(res => {
                    if (res.status==200){                      
                        this.lobeListadoDetalle = res.result.listadoDetalle;
                        this.isDetalle=(this.lobeListadoDetalle.length>0)?true:false;
                        this.PaginarDetalle();
        ...
        ValidarAnularNotaCredito(){
            if(this.row){
                if(this.row.BoletaVenta=="" && this.row.FacturaVenta==""){
                    return false;
                } else {
                    return true;
                }
        ...
        AnularVenta(){
            ...
            //console.log(this.usuario.usuarioLogin, this.usuario.idCliente,this.id,this.row);
            VentaService.AnularVenta(this.usuario.usuarioLogin, this.usuario.idCliente, this.id)
                .then(res => {
                    loading.close();
                    if (res.status==200){       
                        this.$message({
        ...
        AnularVentaNC(){
            ...
            //console.log(this.usuario.usuarioLogin, this.usuario.idCliente,this.id,this.row);
            VentaService.AnularVentaNC(this.usuario.usuarioLogin, this.usuario.idCliente, this.id)
                .then(res => {
                    loading.close();
                    if (res.status==200){   
                        this.$message({
        ...
        BuscarBD(){
            ...
            VentaService.RegistroVentas(this.usuario.usuarioLogin, this.usuario.idCliente, this.isDia,
