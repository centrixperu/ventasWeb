  ...
  <q-page>
    <div class="q-pa-md paddingBottom0">
      <el-container class="padding0">  
                    ...
                    <div class="cus-container-border margin0 width100">
                        <el-main class="padding5">
                            ...
                            <el-row :gutter="5" type="flex" class="paddingTop10">
                                <el-col :xs="7" :sm="6" :md="4" :lg="3" :xl="1">
                                    <div class="cus-label-form-left">
                                        <span class="cus-label textAdjust textAlignL">Módulos:</span>
                                    </div>
                                </el-col>
                                <el-col :xs="17" :sm="18" :md="20" :lg="19" :xl="20">
                                    <div>
                                        <el-button @click="VerModulos()" class="width100" size="small" 
                                                type="primary"> Ver Módulos
                                        </el-button>
                                    </div>
                                </el-col>
                            </el-row>                           
                            ...
                            <el-row :gutter="5" type="flex" class="paddingTop10" v-show="(isNuevo || isEdicion)">
                                <el-col :xs="12" :sm="6" :md="4" :lg="3" :xl="1">
                                    <el-button id="codigo" @click="Guardar()" class="width100" size="small" 
                        ...
                        <el-row :gutter="5" type="flex" class="paddingTop5"
                            ...
                            <el-main class="padding5 cus-border">
                                ...
                                <el-row :gutter="5" type="flex" class="paddingTop5">
                                    <el-col :xs="24" :sm="24">
                                        <div class="cus-label-form-left">
                                            <span class="cus-label textAdjust textAlignL width95P">RUC:</span>
        ...
        <modal-modulos :isModal="isModal" :loModulos="lobeModulo" :loModuloSelec="lobeModuloSelec" 
                        @interface="Cargar">
        </modal-modulos>
...
import AjusteModuloXClienteService from '@/services/ajustes/moduloxcliente-service'
import { mapState,mapGetters } from "vuex";
//MODALES
import ModalModulos from '@/components/pages/ajustes/modals/mdlModulos'
// Importamos JQuery
...
export default {
    ...
    data() {
        return {
            ...
            isModal:false,
            //-----------------------------------
            tittleModulo:'Módulos del Sistema por Cliente',
            //-----------------------------------
            Id:0,
            activo:'SI',
            IdCliente:-1,
    ...
    methods: {
        ...
        CargarDatosIniciales(){
          AjusteModuloXClienteService.CargarDatosIniciales(this.usuario.usuarioLogin, this.usuario.idCliente)
            .then(res => {
                if (res.status==200){
                    this.lobeListado = res.result.loListado;
                    this.lobeCliente = res.result.loCliente;
                    this.lobeModulo = res.result.loModulos;
                    this.lobeModuloSelec = [];
        ...
        ValidarGuardar(){
            if(this.lobeModuloSelec.length==0){
                return 'Debe seleccionar Módulos.';
            }
        ...
        CreaObjetoEnvioBD : function(tipo,id){
            ...
            if (tipo==1){
                ...
                item.loModulos = this.lobeModuloSelec;
            } else if (tipo==2){
                item.Id = id;
        ...
        GuardarBD(){
            AjusteModuloXClienteService.Guardar(this.CreaObjetoEnvioBD(1,0))
                .then(res => {
                    if (res.status==200){
                        this.CargarDatosIniciales();
                        this.$message({
        ...
        ActualizarBD(){
            AjusteModuloXClienteService.Actualizar(this.CreaObjetoEnvioBD(1,0))
                .then(res => {
                    if (res.status==200){
                        this.CargarDatosIniciales();
                        this.$message({
        ...
        Editar(id, row){
            ...
            this.lobeModuloSelec = row.loModulos;
        },
        Eliminar(id, row){
            this.$confirm('Desea eliminar los Módulos del Cliente seleccionado?', 'Eliminar Módulo de Cliente', {
                confirmButtonText: 'SI',
                cancelButtonText: 'NO',
                type: 'warning'
            }).then(() => {
                AjusteModuloXClienteService.Eliminar(this.CreaObjetoEnvioBD(2,id))
                    .then(res => {
                        if (res.status==200){
