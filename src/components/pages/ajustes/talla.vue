                        ...
                        <div>
                            <el-table ref="listado" :data="lobeListadoFiltrado" 
                                ...
                                <el-table-column prop="Nombre" 
                                        width="350">
                                    <template slot="header" slot-scope="scope">
                                        <span class="textAdjust textcenter">Talla</span>
                                        <span class="caret-wrapper">
...
import AjusteTallaService from '@/services/ajustes/talla-service'
...
export default {
    ...
    methods: {
        ...
        CargarDatosIniciales(){
          AjusteTallaService.CargarDatosIniciales(this.usuario.usuarioLogin, this.usuario.idCliente)
            .then(res => {
                if (res.status==200){
        ...
        ValidarGuardar(){
            ...
            if(this.talla==''){
                return 'Debe ingresar Nombre Talla.';
            }
            return '';
        },
        ...
        GuardarBD(){
            AjusteTallaService.Guardar(this.CreaObjetoEnvioBD(1,0))
                .then(res => {
                    if (res.status==200){
                        this.CargarDatosIniciales();
                        this.$message({
        ...
        ActualizarBD(){
            AjusteTallaService.Actualizar(this.CreaObjetoEnvioBD(1,0))
                .then(res => {
                    if (res.status==200){
                        this.CargarDatosIniciales();
                        this.$message({
        ...
        Editar(id, row){
            ...
            //-----------------
            this.Id = row.Id;
            this.talla = row.Nombre;
            this.IdCliente = row.IdCliente;
            this.activo = row.Estado?'SI':'NO';
        },
        Eliminar(id, row){
            this.$confirm('Desea eliminar el Almacén seleccionado?', 'Eliminar Almacén', {
                confirmButtonText: 'SI',
                cancelButtonText: 'NO',
                type: 'warning'
            }).then(() => {
                AjusteTallaService.Eliminar(this.CreaObjetoEnvioBD(2,id))
                    .then(res => {
                        if (res.status==200){
