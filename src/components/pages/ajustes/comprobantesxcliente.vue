                    ...
                    <div class="cus-container-border margin0 width100">
                        <el-main class="padding5">
                            ...
                            <el-row :gutter="5" type="flex" class="paddingTop10">
                                <el-col :xs="7" :sm="6" :md="4" :lg="3" :xl="1">
                                    <div class="cus-label-form-left">
                                        <span class="cus-label textAdjust textAlignL">Comprobantes:</span>
                                    </div>
                                </el-col>
                                <el-col :xs="17" :sm="18" :md="20" :lg="19" :xl="20">
                                    <div>
                                        <el-select v-model="IdTipoDocVenta" filterable multiple placeholder="Select"
                                            size="small" ref="IdTipoDocVenta" class="width100">
                                            <el-option v-for="oTipoDocV in lobeTipoDocVenta" 
                                                :key="oTipoDocV.codigo" id="codigo"
                                                :label="oTipoDocV.descripcion"
                                                :value="oTipoDocV.codigo">
                                            </el-option>                            
                                        </el-select>
                                    </div>
                                </el-col>
                    ...
                    <el-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24">
                        <div>
                            <el-table ref="listado" :data="lobeListadoFiltrado" 
                                ...
                                <el-table-column prop="Comprobantes" 
                                        width="350">
                                    <template slot="header" slot-scope="scope">
                                        <span class="textAdjust textcenter">Comprobantes</span>
                                        <span class="caret-wrapper">
                                ...
                                <el-row :gutter="5" type="flex" class="paddingTop5">
                                    <el-col :xs="24" :sm="24">
                                        <div class="cus-label-form-left width90">
                                            <span class="cus-label textAdjust textAlignL width105P">Comprobantes:</span>
                                            <el-input class="paddingRight10" v-model="oListado.Comprobantes" size="mini" readonly></el-input>
                                        </div>
                                    </el-col>
                                </el-row>
                                <el-row :gutter="5" type="flex" class="paddingTop5">
...
import AjusteComprobanteXClienteService from '@/services/ajustes/comprobantexcliente-service'
...
export default {
    ...
    data() {
        return {
            ...
            nomBuscar:"",
            //-----------------------------------
            tittleModulo:'Comprobantes por Cliente',
            //-----------------------------------
            IdCliente:-1,
            lobeCliente:[],
            IdTipoDocVenta:[],
    ...
    methods: {
        filtrarAscendente(id){
            if(id==0){
                ...
                this.lobeListadoFiltrado = this.lobeListadoFiltrado.sort(function(a, b) {
                    if (a.Comprobantes < b.Comprobantes)
                        return -1;
                    if (a.Comprobantes > b.Comprobantes)
                        return 1;
                    return 0;
        ...
        CargarDatosIniciales(){
          AjusteComprobanteXClienteService.CargarDatosIniciales(this.usuario.usuarioLogin)//, this.usuario.idCliente)
            .then(res => {
                if (res.status==200){
                    this.lobeListado = res.result.loListado;
                    this.lobeCliente = res.result.loCliente;
                    this.lobeTipoDocVenta = res.result.loTipoDocVenta;
                    this.Paginar();
        ...
        ValidarGuardar(){
            ...
            if(this.IdTipoDocVenta.length==0){
                return 'Debe seleccionar Comprobante.';
            }
        ...
        CreaObjetoEnvioBD : function(tipo,id){
            ...
            if (tipo==1){
                ...
                var cont = this.IdTipoDocVenta.length;
                var loComprobante = [];
                for(var i=0;i<cont;i+=1){
                    var obeTienda = {
                        codigo : this.IdTipoDocVenta[i],
                        descripcion: ""
                    };
                    loComprobante.push(obeTienda);
                }
                item.loComprobante = loComprobante;
        ...
        GuardarBD(){
            AjusteComprobanteXClienteService.Guardar(this.CreaObjetoEnvioBD(1,0))
                .then(res => {
                    if (res.status==200){
                        this.CargarDatosIniciales();
                        this.$message({
        ...
        ActualizarBD(){
            AjusteComprobanteXClienteService.Actualizar(this.CreaObjetoEnvioBD(1,0))
                .then(res => {
                    if (res.status==200){
                        this.CargarDatosIniciales();
                        this.$message({
        ...
        Editar(id, row){
            ...
            var cont = row.loComprobante.length;
            var loComprobante = [];
            for(var i=0;i<cont;i+=1){
                loComprobante.push(row.loComprobante[i].codigo);
            }
            this.IdTipoDocVenta = loComprobante;
        },
        Eliminar(id, row){
            this.$confirm('Desea eliminar el Conmprobante seleccionado?', 'Eliminar Comprobante', {
                ...
                AjusteComprobanteXClienteService.Eliminar(this.CreaObjetoEnvioBD(2,id))
                    .then(res => {
                        if (res.status==200){
        ...
        Paginar(){
            ...
            this.totalrows=this.lobeListadoFiltrado.filter(function(item){ 
                return ((
                        ...
                        (item.Comprobantes.toString().toUpperCase().indexOf(this.filtros.filter(f => f.columna=='Comprobantes')[0].Valor.toUpperCase()) >= 0) && 
